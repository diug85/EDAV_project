...
From the k mean analysis obtain the following separation of characters:
* *Main Characters*: As expected Rachel, Monica, Phoebe, Joey, Chandler and Ross constitute one group the has that on average has 1,680 scenes, 8469 lines and 	87,498 words.
...
With the library [`igraph`](https://www.rdocumentation.org/packages/igraph/versions/1.2.4.2){target="_blank"}) we were able to create the adjacency matrix of the characters and quantify the interactions among the 869 characters. For the 6 main characters the adjacency matrix look like this:
...
Also, we Can visualize interactively the relationship between the Main and the Secondary Characters. The width of the graph represents the level of interactions among the characters, as you may perceive the interactions between the main characters is very balanced.
...
[Interactive d3 topic modeling](https://bernacho.github.io/EDAV_project/d3_lda.html)
```{r echo=FALSE}
...
ggplot(episode, aes(x=factor(episodeNumber, levels = 1:25), averageRating)) +
  geom_boxplot() +
...
load("./data/DF_kmeans.RData")
characters_main <- DF_kmeans %>%
  filter(Group=="Main") %>%
  select(character) %>%
  unique()

characters_sec <- DF_kmeans %>%
  filter(Group=="Secondary") %>%
  select(character) %>%
  unique()

# load additional episode detail: dates and viewers
episode_xtra <- read_csv('data/episode_extras.csv')
episode_xtra <- episode %>% left_join(episode_xtra, by='key')
# viewers trend (boxplot)
ggpairs(episode_xtra, c(6,7,11))
```
...
The following boxplot graph shows that the hype for the show and the [main actor's salaries](https://www.quora.com/What-would-be-the-salaries-of-the-characters-on-F-R-I-E-N-D-S) were not backed by the number of people following the weekly episodes.
...
```{r echo=FALSE}
ggplot(episode_xtra %>% filter(viewers<50), aes(seasonNumber, viewers)) +
  geom_boxplot() +
  labs(title = 'Number of viewers (in millions)') +
  xlab('Season') +
  ylab('Viewers') +
  theme_clean()
```
...
```{r echo=FALSE}
ggplot(episode_xtra %>% filter(viewers<50), aes(factor(episodeNumber), viewers)) +
    geom_boxplot() +
  labs(title = 'Number of viewers (in millions)') +
  xlab('Episode') +
  ylab('Viewers') +
...
```{r echo=FALSE}
# filter line counts by character
# load('./data/DF_Character_Episode.RData')
lines_main <- DF_Character_Episode %>%
  filter(character %in% characters_main$character)

aggregate_main <- lines_main %>%
  spread(character, Total_lines, fill=0) %>%
  group_by(episode_id, Avg_Rating) %>%
...
test_matrix <- function(data){
  ...
  for(c in characters){
    y <- as.matrix(data[,c])
    df[c,'Spearman'] <- cor.test(x, y, alternative = 'greater', method = 'spearman')$p.value
    df[c,'Pearson'] <- cor.test(x, y, alternative = 'greater', method = 'pearson')$p.value
  }
  df
}
p_values <-test_matrix(aggregate_main)
```
...
* **Zoom scatterplot** using plotly.
* **Interaction network** using VisNetwork.
* **Bar plot for topics per episode** using d3.
* **Rating time series per season** using d3.


## **VII. Conclusion**

* Rachel and Ross are the characters with most lines, this suggests that most of the stories go around them.
...
* The most frequent terms found in the topic modelling tell us that Friends is a TV show with a very simple language. Words like "Good", "Great", "alright" and "guys" are around the most common.
